...
public class GameUtil {
	...
	public static List<List<Position>> getWinningCombos() {
		...
		wins.add(toList(new Position(1,1), new Position(1,2), new Position(1,3)));
		wins.add(toList(new Position(2,1), new Position(2,2), new Position(2,3)));
		wins.add(toList(new Position(3,1), new Position(3,2), new Position(3,3)));
		
		wins.add(toList(new Position(1,1), new Position(2,1), new Position(3,1)));
		wins.add(toList(new Position(1,2), new Position(2,2), new Position(3,2)));
		wins.add(toList(new Position(1,3), new Position(2,3), new Position(3,3)));
		
		wins.add(toList(new Position(1,1), new Position(2,2), new Position(3,3)));
		wins.add(toList(new Position(3,1), new Position(2,2), new Position(1,3)));
		
		return wins;	
	}

	private static List<Position> toList(Position position, Position position2,
			Position position3) {
	...
	public static boolean matches(Map<Position, Player> moves, Player player,  List<Position> winningCombo) {
		for (Position position : winningCombo) {
			if (moves.get(position) != player) {
				return false;
			}
		}
		return true;
	...
	public static boolean isWinner(Map<Position, Player> moves, Player player) {
		for (List<Position> combo: getWinningCombos()) {
			if (matches(moves, player, combo)) {
				return true;
			}
		}
		return false;
