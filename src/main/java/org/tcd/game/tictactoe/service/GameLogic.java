...
public class GameLogic {
    ...
    Position sandwichOpen(Position position1, Position position2) {
        ...
        if (isOpen(position1) && (Player.O == game.playerAt(middle))
    ...
    Position fork() {
        Optional<Position> opt = getOpenPositions().stream()
    ...
    Position blockFork() {
        if (game.getMoves().size() == 3) {
            ...
            } else if (isInPosition(Player.X, 1, 3)
                    && isInPosition(Player.X, 3, 1)) {
                return new Position(1, 2);
            } else if (isInPosition(Player.X, 3, 2)
                    && isInPosition(Player.X, 2, 3)) {
